...
const champions = Object.keys(require(path.join(__dirname, 'fixtures/championgg/responses/champions.json')).data).sort()

const RESPONSES_FIXTURES = {}
glob.sync(path.join(__dirname, 'fixtures/championgg/responses/*.html')).forEach(fixture => {
  RESPONSES_FIXTURES[path.basename(fixture).replace('.html', '')] = fs.readFileSync(fixture, 'utf8')
})

const RESULTS_FIXTURES = {}
glob.sync(path.join(__dirname, 'fixtures/championgg/results/*.json')).forEach(fixture => {
...
function testWithFixture (fixture) {
  return championgg.getSr()
    .then(() => {
      const results = [].concat(...store.get('sr_itemsets'))
