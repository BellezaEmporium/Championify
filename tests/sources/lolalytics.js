...
glob.sync(path.join(__dirname, 'fixtures/lolalytics/responses/*.json')).forEach(fixture => {
  RESPONSES_FIXTURES[path.basename(fixture).replace('.json', '')] = require(fixture)
})

const RESULTS_FIXTURES = {}
glob.sync(path.join(__dirname, 'fixtures/lolalytics/results/*.json')).forEach(fixture => {
