...
glob.sync(path.join(__dirname, 'fixtures/leagueofgraphs/responses/*.html')).forEach(fixture => {
  RESPONSES_FIXTURES[path.basename(fixture).replace('.html', '')] = fs.readFileSync(fixture, 'utf8')
})

const RESULTS_FIXTURES = {}
glob.sync(path.join(__dirname, 'fixtures/leagueofgraphs/results/*.json')).forEach(fixture => {
